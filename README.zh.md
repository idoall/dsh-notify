...
- **任务噪声控制**：子任务、后台任务与工作流完成通知默认关闭。
- **在线版本检测**：设置页会显示当前版本与 npm 上最新版本，并提供**检查更新**、GitHub、更新日志、反馈 Issue 入口。检测只读：宿主只查询 registry 并汇报，同时给出可复制的升级命令，绝不自动安装、也绝不重启 DSH。registry 不可达时显示「检查失败」，而不会把当前版本谎称为最新。
