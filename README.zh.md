...
- **堆叠与折叠**：角上常驻五张，新卡入场时把旧卡平滑往下推；**在等你的那张一定在其中**，其余的留在本页、以「+N」计数藏在后面。悬浮（或点「+N」）把整条队列展开成可滚动的卡片列，最新在上、一条都不丢。页面开着期间都会保留，刷新即清空。
...
<p align="center">
  <img src="./assets/panel.png" width="46%" alt="五条通知折叠成一张卡片加一个「+N」计数，其余的在其下方露出边缘">
</p>
...
| 自测 | — | 只有一条通道，所以只有页面浮层这一项：**测试一条**、**测试 5 条**（正好填满角上那一窗）、**测试 8 条**（多出 3 条，看「+N」和展开后的滚动列）。页面加载**不会**自动发送测试。 |
