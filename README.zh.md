...
会话完成一轮、运行失败、向你提问或请求审批时，当前页面右上角弹出浮层；侧边栏的铃铛保存历史：未读/已读分栏、点一下跳回产生该通知的会话，提问与审批还能**直接在浮层里回答**。
...
- **铃铛历史**：未读/已读分栏、每条带会话名与相对时间、分页、「全部已读」，批量删除藏在「**选择…**」后面（没有任何破坏性操作是一点即中的）。
...
1. 跑一个任务。某轮完成、失败或向你提问时，右上角出现浮层，铃铛出现未读角标。
2. 点浮层正文跳到对应会话；点选项就地回答；点 **×** 关闭（已结束的通知会被标记已读，**还在等你回复的提问不会**）。
3. 打开铃铛看历史。点任意一条跳到它的会话与轮次；用「**选择…**」勾选后删除选中，或全部删除。
4. 切到别的标签继续干活：有未读时标题会闪，任务完成时会响提示音——即使 DSH 页面在后台。
...
当前发布：插件 **`0.1.0`** 已在 DeepSeek Harness **`0.1.5-rc.1`** 上验证。
