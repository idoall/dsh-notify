...
- **一扇窗，不是叠压**：桌面 **1～5 条全部完整展开**（窄屏/触屏是 1～3 条），不会折叠、不会重叠——一次来 5 条和从 1 条加到 5 条一样。**第 6 条**起才排到窗下面，滚动或手指滑动就能看到，窗沿会露出下一张的边。角上的计数是**本页全部**条数（8 条就是「+8」），不是藏起来的那些；点它可以在队列两端之间跳转。页面开着期间一条都不丢，刷新即清空。
