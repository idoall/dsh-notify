...
- The settings self-test can fire the window-sized group (**测试 5 条**) and an overflowing one (**测试 8 条**), so what happens past the window — the `+N` count and the scrollable expand — can be looked at without waiting for eight real notifications.
...
- **The corner is a window onto a page-scoped queue.** It shows five cards — always including anything waiting on the user, which is ordered first — and the rest stay in the page behind the `+N` count instead of being dropped. Hovering (or clicking the count) expands the whole queue into a scrollable column, and a card on its way out no longer counts towards the window while it slides away. A refresh still clears everything: nothing is stored, here or on the host.
...
- A collapsed deck leaked text out from under the front card. The layers behind it are the same cards at a 10px offset, so a notification taller than the one in front showed a line of its own body text below it. A deeper layer now keeps its surface — the border, radius and background that read as a stack — and hides its contents, so the deck is one readable card plus edges, which is what the `+N` count already says.
- The expanded stack flickered when the pointer crossed the gap between two cards. Both the card and the container listened for the pointer, so leaving a card for the 12px gap counted as leaving the stack, which collapsed it — and collapsing slid the cards back under the cursor, which expanded it again. The gesture now belongs to the container alone, whose box spans the whole column including the gaps. The suite dispatches the real boundary event (`pointerout` from a card with the container as `relatedTarget`) and fails if a card ever listens for the pointer again.
